...
function isOverIcon(posX, posY, inset, theme, h) {
  return posX >= inset + theme.cellHorizontalPadding - 4 && posX <= inset + theme.cellHorizontalPadding + 18 && posY >= h / 2 - 9 && posY <= h / 2 + 9;
}
const renderer = {
  ...
  draw: (args, cell) => {
    ...
    if (canOpen) {
      ...
      ctx.lineWidth = 2;
      ctx.stroke();
      if (overIcon) (_args$overrideCursor = args.overrideCursor) === null || _args$overrideCursor === void 0 || _args$overrideCursor.call(args, "pointer");
    }
