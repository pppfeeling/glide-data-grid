...
**2024년 수정 이력**: 원래 데미지 경로에서는 `drawHighlightRings`가 호출되지 않았습니다. 이로 인해 `clipLeftPx`를 사용하는 커스텀 셀(tree-cell 등)에서 데미지 렌더링 시 포커스 링의 왼쪽 보더가 사라지는 버그가 있었습니다. 수정하여 데미지 렌더링 후에도 하이라이트 링을 다시 그리도록 했습니다.
