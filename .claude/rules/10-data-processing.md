...
`rowGrouping`은 `<DataEditor />`에 전달하는 설정 객체:
- `groups`: 그룹 헤더 정의 배열 (`headerIndex`, `isCollapsed`)
- `height`: 그룹 헤더 행 높이 (px)
- `themeOverride`: 그룹 헤더 전용 테마
- `navigationBehavior`: 키보드 탐색 시 그룹 헤더 건너뛰기 설정
- `selectionBehavior`: 그룹 헤더 가로지르는 범위 선택 제어

### useRowGrouping Hook

`rowGrouping` 설정 기반 인터랙션 헬퍼:
- **`mapper(item)`**: 시각적 행 인덱스 ↔ 원본 데이터 인덱스 변환. `isGroupHeader`, `originalIndex`, `path` 반환
- **`updateRowGroupingByPath`**: 특정 그룹의 `isCollapsed` 불변 업데이트
- **`getRowGroupingForPath`**: 특정 그룹의 현재 설정 조회

### 데이터 처리 워크플로우

`rowGrouping`은 순수 시각적 표현(Presentation Layer)만 담당. 집계(sum, count)는 개발자가 사전 처리:

1. **데이터 정렬**: 그룹화 기준으로 sort
2. **집계 계산**: 각 그룹의 count, sum 등 미리 계산
3. **데이터 재구성**: 그룹 시작점에 헤더용 데이터 객체 삽입
4. **rowGrouping prop 생성**: 헤더 인덱스 배열로 `groups` 구성
5. **getCellContent**: `mapper`로 그룹 헤더/일반 행 판별 후 적절한 데이터 반환

### headerIndex 동작 방식

`headerIndex`는 화면 위치가 아닌 **원본 데이터 인덱스**. `groups` 배열의 첫 번째 항목이 visual index 0이 되며, 그 이전 데이터는 표시되지 않음.
