...
| 컴포넌트 | 파일 | 역할 |
|----------|------|------|
| DataEditorImpl | `data-editor.tsx` | 최상위 오케스트레이터. props 전달, 선택/편집/키보드/복사붙여넣기 로직 총괄 |
| DataEditorContainer | `data-grid-container.tsx` | 레이아웃 래퍼. width/height 스타일 적용 |
| DataGridSearch | `data-grid-search.tsx` | 검색 UI 및 하이라이트 로직 |
...
Row marker 컬럼 추가 시 사용자 컬럼 인덱스와 내부 인덱스가 달라짐:
...
- 외부 콜백 호출 시: `-rowMarkerOffset` (내부→외부)
- 내부 처리 시: `+rowMarkerOffset` (외부→내부)
- `getMangledCellContent`: 좌표에 따라 marker/status/ID/사용자 데이터 셀 반환
- `mangledCols`: marker 컬럼들을 사용자 컬럼 앞에 추가한 전체 컬럼 배열
